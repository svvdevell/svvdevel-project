        ...
        <div class="container">
            ...
            <div v-if="!loading && !error" class="cars-grid">
                <div v-for="car in cars" :key="car.id" class="car-card">
                    <!-- Car Image -->
                    <div class="car-image" @click="openCarModal(car)">
                        <img v-if="car.imageCount > 0"
                            :src="`/uploads/car_sale_${car.id}_image_1_${getImageTimestamp(car.createdAt)}.jpg`"
                    ...
                    <div class="car-info">
                        ...
                        <div class="car-footer">
                            <span class="date">Добавлено: {{ formatDate(car.createdAt) }}</span>
                            <button @click="openCarModal(car)" class="view-btn">Подробнее</button>
                        </div>
                    </div>
                </div>
            </div>
...
const fetchFilteredCars = async () => {
    ...
    try {
        ...
        // If no filters applied, fetch from server
...
const getImageTimestamp = (createdAt) => {
    // Приближенная оценка timestamp для первого изображения
